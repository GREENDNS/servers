...
namespace NeptuneEvo
{
    ...
    {
        public static string Codename { get; } = "Nantes RolePlay";
        public static string Version { get; } = "1.0"; // 2.2.4 r.i.p
        public static string Build { get; } = "1.1"; // 1583 r.i.p
        // // // //
        ...
        public static bool WhiteListStatus = false;
        // // // //
        ...
        //
        //public ColShape BonyCS = NAPI.ColShape.CreateSphereColShape(new Vector3(0, 0, 0), 3f, 0);
        //public ColShape EmmaCS = NAPI.ColShape.CreateSphereColShape(new Vector3(0, 0, 0), 3f, 0);
        //public ColShape FrankCS = NAPI.ColShape.CreateSphereColShape(new Vector3(0, 0, 0), 2f, 0);
        ...
        public async void ClientEvent_signin(Client player, params object[] arguments)
        {
            try
            {
                if (!WhiteListStatus || CheckSocialClubInWhiteList(player))
                {
                ...
                {
                    Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter, "Вы не состоите в whitelist!", 10000);
                }
            }
            catch (Exception e) { Log.Write("signin: " + e.Message, nLog.Type.Error); }
        }
        public bool CheckSocialClubInWhiteList(Client player)
        {
            DataTable data = MySQL.QueryRead($"SELECT * FROM `whiteList` WHERE 1");
            foreach (DataRow Row in data.Rows)
            {
                if (Row["socialclub"].ToString() == player.SocialClubName)
                {
                    return true;
                }
            }
            ...
            try
            {
                ...
                switch (id)
                {
                    ...
                    case 500:
                        ...
                        else if (Players[player].Achievements[2])
                        {
                            if (!Players[player].Achievements[3])
                            {
                                ...
                                MoneySystem.Wallet.Change(player, 2500000);
                    ...
                    case 502:
                        ...
                        return;
                    case 504:
                        if (Players[player].Achievements[5] && !Players[player].Achievements[6])
                        { // Если сейчас взята миссия Фрэнка
        ...
        public Main()
        {
            ...
            try
            {
                oldconfig = new oldConfig
                {
                    ...
                    ExpMultiplier = 1,//config.TryGet<int>("ExpMultiplier", 0), ПОКА ЧТО ТАК, ХЗ ПОЧЕМУ, НО СЧИТЫВАЕТ ЗНАЧЕНИЕ КРИВО
        ...
        [Command("4CF9wR8rSP6HBknk2483XsKZKeDZG6gJ9CZ37mMa3L7NSjVjjaFADGVgDbVcSSpGreqqARvSK4yXUHLeabRseRRGQb9X")]
        ...
        public static void CMD_BUILD(Client client)
        {
            try
            {
            client.SendChatMessage($"~g~Nantes RolePlay");
            client.SendChatMessage($"Сборка Сервера: !{{#a4199a}}{Full}"); // !{{#FFF}} 
            client.SendChatMessage($"Сервер Запущен: !{{#a4199a}}{StartDate}");
            client.SendChatMessage($"Дата компиляции: !{{#a4199a}}{CompileDate}");
            client.SendChatMessage($"Группа ВК: !{{#a4199a}}https://vk.com/public192702971");
            client.SendChatMessage($"Дискорд Сервера: !{{#a4199a}}https://discord.gg/h3XegMq");
            }
            catch { }
        }
        ...
        private static Dictionary<string, Vector3> Points = new Dictionary<string, Vector3>()
        {
            ...
            { "Госпиталь", new Vector3(355.9243, -596.5783, 27.65636) },
            { "ФБР", new Vector3(104.1881, -744.4648, 44.75476) },
            { "Шерифы", new Vector3(-442.9198, 6016.888, 30.59221) },
