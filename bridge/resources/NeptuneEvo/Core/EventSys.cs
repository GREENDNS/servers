...
namespace NeptuneEvo.Core
{
    class EventSys : Script
    {
            ...
            if (Main.Players.ContainsKey(player))
            {
                ...
                if (AdminEvent.EventState == 0)
                {
                    if (eventname.Length < 50)
                    {
                        ...
                        AddAdminEventLog();
                        NAPI.Chat.SendChatMessageToAll("!{#a4199a}Дорогие наши игроки проекта, сейчас начнётся интересное мероприятие '" + eventname + "'!");
                        if (members != 0) NAPI.Chat.SendChatMessageToAll("!{#a4199a}На нашем мероприятии установлен лимит игроков: " + members + ".");
                        else NAPI.Chat.SendChatMessageToAll("!{#a4199a}На нашем мероприятии не установлен лимит игроков.");
                        if (AdminEvent.Zone != null) NAPI.Chat.SendChatMessageToAll("!{#a4199a}Мероприятие будет действовать в зоне " + radius + "м от точки телепорта.");
                        NAPI.Chat.SendChatMessageToAll("!{#a4199a}Для того чтобы вы телепортировались на мероприятие , пропишите команду/mp , после чего вы телепортирует на мероприятие , ведите себя адекватно , желаем удачи и хорошего настроения !");
                        }
            ...
            if (Main.Players.ContainsKey(player))
            {
                ...
                if (AdminEvent.EventState == 1)
                {
                    if (AdminEvent.EventMembers.Count >= 1)
                    {
                        ...
                        NAPI.Chat.SendChatMessageToAll("!{#337FA8}Мероприятие '" + AdminEvent.Name + "' началось, телепорт закрыт!");
                        NAPI.Chat.SendChatMessageToAll("!{#337FA8}Игроков на мероприятии: " + AdminEvent.EventMembers.Count + ".");
                    }
        ...
        private void CloseAdminEvent(Client winner, uint reward)
        {
            ...
            NAPI.Chat.SendChatMessageToAll("!{#337FA8}Мероприятие '" + AdminEvent.Name + "' закончилось, спасибо за участие!");
            if (winner != AdminEvent.Admin)
            {
                if (reward != 0)
                {
                    NAPI.Chat.SendChatMessageToAll("!{#337FA8}Победитель " + winner.Name + " получил приз в размере " + reward + "$.");
                    MoneySystem.Wallet.Change(winner, (int)reward);
                }
                else NAPI.Chat.SendChatMessageToAll("!{#337FA8}Победитель: " + winner.Name + ".");
            }
        }
    }
}
