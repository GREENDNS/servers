...
namespace NeptuneEvo.Core
{
    class Commands : Script
    {
        ...
        [Command("givelic")]
        public static void CMD_giveLicense(Client player, int id, int lic)
        {
            try
            {
        ...
        [Command("addpromo")]
        public static void CMD_addPromo(Client player, int uuid, string promocode)
        {
            try
            {
        ...
        public static void CMD_startMatWars(Client player)
        {
            try
            {
                ...
                {
                    player.SendChatMessage("~r~Война за материалы уже идёт");
                    return;
                }
                Fractions.MatsWar.startMatWarTimer();
                player.SendChatMessage("~r~Начата война за материалы");
                GameLog.Admin($"{player.Name}", $"startMatwars", $"");
            }
            catch (Exception e) { Log.Write("startmatwars: " + e.Message, nLog.Type.Error); }
        }
        [Command("whitelist")]
        public static void CMD_whitelist(Client player, string status)
        {
            try
            {
                if(status == "on")
                {
                    Main.WhiteListStatus = true;
                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, "White list включен!", 3000);
                }
                if(status == "off")
                {
                    Main.WhiteListStatus = false;
                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, "White list выключен!", 3000);
                }
                GameLog.Admin($"{player.Name}", $"whitelist", $"");
            }
            catch (Exception e) { Log.Write("whitelist: " + e.Message, nLog.Type.Error); }
        }
        [Command("whitelistkick")]
        public static void CMD_whitelistkick(Client player)
        {
            try
            {
                if (Main.WhiteListStatus)
                {
                    ...
                    foreach (var p in all_players)
                    {
                        if (p == null) continue;
                        if (!Main.Players.ContainsKey(p)) continue;
                        if (!CheckSocialClubInWhiteList(p.Name))
                        {
                            Notify.Send(p, NotifyType.Warning, NotifyPosition.BottomCenter, "White list был включен!", 3000);
                            p.Kick();
                        }
                    }
                }
                ...
                {
                    Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter, "White list выключен!", 3000);
                }
                GameLog.Admin($"{player.Name}", $"whitelistkick", $"");
            }
            catch (Exception e) { Log.Write("whitelistkick: " + e.Message, nLog.Type.Error); }
        }
        [Command("whitelistdel")]
        public static void CMD_whitelistdel(Client player, string socialClub)
        {
            try
            {
        ...
        public static void CMD_whitelistadd(Client player, string socialClub)
        {
            try
            {
        ...
        [Command("newjobveh")]
        public static void newjobveh(Client player, string typejob, string model, string number, int c1, int c2)
        {
            try
            {
        ...
        [Command("newfracveh")]
        public static void ACMD_newfracveh(Client player, string model, int fracid, string number, int c1, int c2) // add rank, number
        {
            try
            {
        ...
        public static void CMD_adminLSnewsChat(Client player, string message)
        {
            try
            {
        ...
        [Command("tpcar")]
        public static void CMD_teleportToMeWithCar(Client player, int id)
        {
            try
            {
                Client Target = Main.GetPlayerByID(id);
        ...
        public static void CMD_setWeather(Client player, byte weather)
        {
            ...
            GameLog.Admin(player.Name ?? "", string.Format("setWeather({0})", (object) weather), "");
        }
        ...
        //[Command("position")]
        //public static void position(Client player)
        //{
        //    try
        //    {
        ...
        [Command("givearmylic")]
        public static void CMD_GiveArmyLicense(Client player, int id)
        {
            try
            {
                var target = Main.GetPlayerByID(id);
