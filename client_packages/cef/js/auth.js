...
$(document).ready(() => {
    ...
    authBtn.on('click', () => {
        ...
        delete authData['action'];
        delete authData['new-user__email'];
        delete authData['new-user__login'];
        delete authData['new-user__pw'];
        delete authData['new-user__pw-repeat'];
        delete authData['new-user__promo-code'];
    ...
    endRegisterBtn.on('click', () => {
        let regData = $('#auth-form').toJSON();
        regData = JSON.parse(regData)
        delete regData['action'];
        delete regData['entry-login'];
        delete regData['entry-password'];
    ...
    endRestoreBtn.on('click', (e) => {
        ...
        delete regData['action'];
        delete regData['entry-password'];
        delete regData['new-user__email'];
        delete regData['new-user__login'];
        delete regData['new-user__pw'];
        delete regData['new-user__pw-repeat'];
        delete regData['new-user__promo-code'];
        regData = JSON.stringify(regData);
        let myval = document.getElementById("auth-form").elements[0].value;
        if (!restPassState) {
            e.preventDefault();
            if (myval.length != 0) {
                restPassState = true;
